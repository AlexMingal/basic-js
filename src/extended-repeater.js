...
function repeater(str, options) {
  ...
  if (options.additionRepeatTimes) {
    ...
    for (let i = 0; i < options.additionRepeatTimes; i++) {
      i < options.additionRepeatTimes - 1 ? additions.push(options.addition + options.additionSeparator) : additions.push(options.addition)
    }
    options.addition = additions.join('');
  }
  str = options.addition ? str + options.addition : str;
  for (let i = 0; i < options.repeatTimes; i++) {
    i < options.repeatTimes - 1 ? res.push(str + options.separator) : res.push(str);
  }
  return res.join('');
}
